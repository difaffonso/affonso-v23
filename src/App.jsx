...
var HINTS={dash:"Visão geral da clínica: indicadores do dia, pendências e atalhos para as áreas mais usadas.",agenda:"Agendamento com confirmação automática, controle de faltas e remarcações. Toque num horário para abrir.",pacs:"Fichas completas: anamnese digital, histórico, documentos e orçamentos de cada paciente.",remarcar:"Pacientes que faltaram ou desmarcaram, prontos para reagendar com um toque.",pros:"Controle de próteses no laboratório: envio, retorno, instalação e refações.",impl:"Acompanhamento de implantes: cirurgias, enxertos, próteses e retornos mês a mês.",lems:"Lembretes e tarefas da equipe, com avisos automáticos de aniversários e retornos.",conversas:"Mensagens recebidas pelo WhatsApp da clínica, ligadas a cada paciente.",fin:"Financeiro da clínica: recebimentos, despesas, comissões e fluxo de caixa.",rel:"Relatórios de faturamento, produção por dentista e desempenho da clínica.",desp:"Controle de gastos fixos e variáveis, da clínica e pessoais.",stk:"Estoque de materiais e implantes, com alerta de mínimo e movimentações.",pixdent:"Pix recebido direto pelos dentistas, para acerto de comissões.",pdent:"Acompanhamento dos tratamentos e produção de cada dentista.",rec:"Emissão de receituário e atestados com os dados da clínica.",orient:"Orientações pré e pós-procedimento para enviar ao paciente.",audit:"Verificação de inconsistências nos dados da clínica.",escala:"Agenda de plantão do mês inteiro. Toque num dia para ver/editar só aquele dia; em \u201cEscala fixa\u201d defina os dias fixos de cada profissional (ex.: toda terça e quinta).",adm:"Área administrativa: usuários, procedimentos, valores, dados da clínica e configurações."};
function PersonEditor({target,fixos,deltas,onClose,onSaveRules,addDay,delExtra,pColor,pKey}){
var tipo=target.tipo,pid=target.pid,nome=target.nome;
var same=function(r){return pid!=null?(r.pid===pid):(r.pid==null&&(r.nome||"")===(nome||""));};
var initRules=fixos.filter(function(r){return r.tipo===tipo&&same(r);}).map(function(r){return {dias:(r.dias||[]).slice(),inicio:r.inicio,fim:r.fim};});
if(!initRules.length)initRules=[{dias:[],inicio:"08:00",fim:"18:00"}];
var [rules,setRules]=useState(initRules);
var initAv=[];Object.keys(deltas||{}).forEach(function(ds){(((deltas[ds]||{}).add)||[]).forEach(function(e){if(e.tipo===tipo&&(pid!=null?e.pid===pid:(e.pid==null&&(e.nome||"")===(nome||"")))){initAv.push({ds:ds,inicio:e.inicio,fim:e.fim,addId:e.id});}});});
var [avNew,setAvNew]=useState([]);
var [removed,setRemoved]=useState({});
var [avForm,setAvForm]=useState(false);
var [avf,setAvf]=useState({ds:"",inicio:"08:00",fim:"12:00"});
var DL=["D","S","T","Q","Q","S","S"];
var toggleDia=function(ri,d){setRules(function(rs){return rs.map(function(r,i){if(i!==ri)return r;var dd=r.dias.slice();var k=dd.indexOf(d);if(k>=0)dd.splice(k,1);else dd.push(d);return Object.assign({},r,{dias:dd});});});};
var setHr=function(ri,patch){setRules(function(rs){return rs.map(function(r,i){return i===ri?Object.assign({},r,patch):r;});});};
var addRule=function(){setRules(function(rs){return rs.concat([{dias:[],inicio:"08:00",fim:"12:00"}]);});};
var delRule=function(ri){setRules(function(rs){return rs.filter(function(x,i){return i!==ri;});});};
var fmt=function(ds){var a=ds.split("-");return a[2]+"/"+a[1];};
var save=function(){onSaveRules(tipo,pid,nome,rules);initAv.forEach(function(a){if(removed[a.addId])delExtra(a.ds,a.addId);});avNew.forEach(function(a){addDay(a.ds,{tipo:tipo,pid:pid,nome:nome,inicio:a.inicio,fim:a.fim});});onClose();};
var ti={border:"1.5px solid "+G.border,borderRadius:7,padding:"6px",fontSize:12.5,outline:"none",width:84,boxSizing:"border-box"};
return <Modal open={true} close={onClose} title={nome||"Profissional"} ch={<div style={{display:"flex",flexDirection:"column",gap:14}}>
<div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:34,height:34,borderRadius:"50%",background:pColor(pKey(tipo,pid,nome)),color:"#fff",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700}}>{((nome||"?")[0]||"?").toUpperCase()}</span><div><div style={{fontSize:15,fontWeight:700,color:G.text}}>{nome}</div><div style={{fontSize:11,color:G.muted,fontWeight:600}}>{tipo==="dent"?"Dentista":"Recepção / equipe"}</div></div></div>
<div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,letterSpacing:".5px",marginBottom:8}}>DIAS FIXOS NA SEMANA</div>
<div style={{display:"flex",flexDirection:"column",gap:10}}>{rules.map(function(r,ri){return <div key={ri} style={{background:G.bg,borderRadius:11,padding:"10px"}}>
<div style={{display:"flex",gap:4,marginBottom:8}}>{DL.map(function(d,di){var on=r.dias.indexOf(di)>=0;return <button key={di} onClick={function(){toggleDia(ri,di);}} style={{flex:1,border:"1.5px solid "+(on?G.primary:G.border),background:on?G.primary:"#fff",color:on?"#fff":G.text,borderRadius:8,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer"}}>{d}</button>;})}</div>
<div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span style={{fontSize:11,color:G.muted,fontWeight:700}}>Entra</span><input type="time" value={r.inicio} onChange={function(e){setHr(ri,{inicio:e.target.value});}} style={ti}/><span style={{fontSize:11,color:G.muted,fontWeight:700}}>Sai</span><input type="time" value={r.fim} onChange={function(e){setHr(ri,{fim:e.target.value});}} style={ti}/>{rules.length>1&&<button onClick={function(){delRule(ri);}} style={{marginLeft:"auto",border:"none",background:"#FDECEA",color:G.red,borderRadius:7,padding:"6px 9px",fontSize:13,cursor:"pointer",fontWeight:700}}>🗑</button>}</div>
</div>;})}</div>
<button onClick={addRule} style={{marginTop:8,border:"1.5px dashed "+G.primary,background:"#fff",color:G.primary,borderRadius:9,padding:"9px",fontSize:12,fontWeight:700,cursor:"pointer",width:"100%"}}>+ Adicionar outro horário (ex.: sáb 08–12)</button></div>
<div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,letterSpacing:".5px",marginBottom:8}}>DIAS AVULSOS (PONTUAIS)</div>
<div style={{display:"flex",flexWrap:"wrap",gap:6}}>
{initAv.filter(function(a){return !removed[a.addId];}).map(function(a){return <span key={"i"+a.addId} style={{display:"inline-flex",alignItems:"center",gap:6,background:G.accent,borderRadius:8,padding:"6px 10px",fontSize:11.5,color:G.primary,fontWeight:600}}>{fmt(a.ds)+" · "+a.inicio+"–"+a.fim}<button onClick={function(){setRemoved(function(p){var n=Object.assign({},p);n[a.addId]=true;return n;});}} style={{border:"none",background:"transparent",color:G.primary,fontSize:13,fontWeight:700,cursor:"pointer",padding:0}}>✕</button></span>;})}
{avNew.map(function(a,ai){return <span key={"n"+ai} style={{display:"inline-flex",alignItems:"center",gap:6,background:G.accent,borderRadius:8,padding:"6px 10px",fontSize:11.5,color:G.primary,fontWeight:600}}>{fmt(a.ds)+" · "+a.inicio+"–"+a.fim}<button onClick={function(){setAvNew(function(p){return p.filter(function(x,i){return i!==ai;});});}} style={{border:"none",background:"transparent",color:G.primary,fontSize:13,fontWeight:700,cursor:"pointer",padding:0}}>✕</button></span>;})}
<button onClick={function(){setAvf({ds:"",inicio:"08:00",fim:"12:00"});setAvForm(true);}} style={{border:"1.5px dashed "+G.primary,background:"#fff",color:G.primary,borderRadius:8,padding:"6px 10px",fontSize:11.5,fontWeight:700,cursor:"pointer"}}>+ dia específico</button></div>
{avForm&&<div style={{marginTop:10,background:G.bg,borderRadius:11,padding:"10px",display:"flex",flexDirection:"column",gap:8}}>
<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><input type="date" value={avf.ds} onChange={function(e){setAvf(function(p){return Object.assign({},p,{ds:e.target.value});});}} style={{border:"1.5px solid "+G.border,borderRadius:7,padding:"6px",fontSize:12.5,outline:"none"}}/><input type="time" value={avf.inicio} onChange={function(e){setAvf(function(p){return Object.assign({},p,{inicio:e.target.value});});}} style={ti}/><span style={{color:G.muted}}>–</span><input type="time" value={avf.fim} onChange={function(e){setAvf(function(p){return Object.assign({},p,{fim:e.target.value});});}} style={ti}/></div>
<div style={{display:"flex",gap:8}}><button onClick={function(){if(!avf.ds)return;setAvNew(function(p){return p.concat([{ds:avf.ds,inicio:avf.inicio,fim:avf.fim}]);});setAvForm(false);}} style={{flex:1,border:"none",background:G.primary,color:"#fff",borderRadius:8,padding:"8px",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>Adicionar dia</button><button onClick={function(){setAvForm(false);}} style={{border:"1.5px solid "+G.border,background:"#fff",color:G.muted,borderRadius:8,padding:"8px 12px",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>Cancelar</button></div></div>}
</div>
<Btn ch="Salvar" onClick={save}/>
</div>}/>;
}
function Escala({dents,users,user,escala,setEscala}){var isAdmin=user.level>=3;
var MES=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
var PAL=["#1E8449","#2E5C86","#B26A1C","#7A4FA3","#0E7C66","#9B2C2C","#3F6212","#7C3AED"];
var pColor=function(key){var h=0,i;for(i=0;i<(key||"").length;i++){h=(h*31+key.charCodeAt(i))>>>0;}return PAL[h%PAL.length];};
var pKey=function(tipo,pid,nome){return pid!=null?("p"+tipo+pid):("n"+(nome||""));};
var first=function(nome){return ((nome||"").trim().split(/\s+/)[0]||"");};
var samePerson=function(r,pid,nome){return pid!=null?(r.pid===pid):(r.pid==null&&(r.nome||"")===(nome||""));};
...
var fixos=(escala&&escala.fixos)||[];var deltas=(escala&&escala.deltas)||{};
var [cur,setCur]=useState({y:now.getFullYear(),m:now.getMonth()});
var [selDate,setSelDate]=useState(null);
var [listOpen,setListOpen]=useState(false);
var [edit,setEdit]=useState(null);
var [dayAdd,setDayAdd]=useState(null);
var [af,setAf]=useState({pid:"",nome:"",inicio:"08:00",fim:"18:00"});
var resolve=function(ds){var a=ds.split("-");var wd=new Date(+a[0],+a[1]-1,+a[2]).getDay();var dl=deltas[ds]||{};var off=dl.off||[];var hrs=dl.hrs||{};var base=fixos.filter(function(r){return (r.dias||[]).indexOf(wd)>=0&&off.indexOf(r.id)<0;}).map(function(r){var h=hrs[r.id]||{};return {key:"fx"+r.id,ruleId:r.id,fixo:true,tipo:r.tipo,pid:r.pid,nome:r.nome,inicio:h.inicio||r.inicio,fim:h.fim||r.fim};});var extra=(dl.add||[]).map(function(e){return {key:"ex"+e.id,addId:e.id,fixo:false,tipo:e.tipo,pid:e.pid,nome:e.nome,inicio:e.inicio,fim:e.fim};});return base.concat(extra).sort(function(a,b){return (a.inicio||"").localeCompare(b.inicio||"");});};
var hasDelta=function(ds){var d=deltas[ds];return !!(d&&((d.add&&d.add.length)||(d.off&&d.off.length)||(d.hrs&&Object.keys(d.hrs).length)));};
var patchDelta=function(ds,fn){setEscala(function(prev){var dl=Object.assign({},(prev&&prev.deltas)||{});var c=Object.assign({add:[],off:[],hrs:{}},dl[ds]||{});c.add=(c.add||[]).slice();c.off=(c.off||[]).slice();c.hrs=Object.assign({},c.hrs||{});fn(c);if(!c.add.length&&!c.off.length&&!Object.keys(c.hrs).length){delete dl[ds];}else{dl[ds]=c;}return {fixos:((prev&&prev.fixos)||[]).slice(),deltas:dl};});};
var addDay=function(ds,e){patchDelta(ds,function(c){c.add.push(Object.assign({id:Date.now()+Math.floor(Math.random()*1e6)},e));});};
var delFixo=function(ds,rid){patchDelta(ds,function(c){if(c.off.indexOf(rid)<0)c.off.push(rid);delete c.hrs[rid];});};
var delExtra=function(ds,aid){patchDelta(ds,function(c){c.add=c.add.filter(function(e){return e.id!==aid;});});};
var setFixoHr=function(ds,rid,patch){patchDelta(ds,function(c){c.hrs[rid]=Object.assign({},c.hrs[rid]||{},patch);});};
var setExtraHr=function(ds,aid,patch){patchDelta(ds,function(c){c.add=c.add.map(function(e){return e.id===aid?Object.assign({},e,patch):e;});});};
var restoreDay=function(ds){setEscala(function(prev){var dl=Object.assign({},(prev&&prev.deltas)||{});delete dl[ds];return {fixos:((prev&&prev.fixos)||[]).slice(),deltas:dl};});};
var saveFixos=function(tipo,pid,nome,rules){setEscala(function(prev){var pf=((prev&&prev.fixos)||[]).filter(function(r){return !(r.tipo===tipo&&samePerson(r,pid,nome));});var base=Date.now();var add=rules.filter(function(rl){return rl.dias&&rl.dias.length;}).map(function(rl,i){return {id:base+i+Math.floor(Math.random()*1e5),tipo:tipo,pid:pid,nome:nome,dias:rl.dias.slice(),inicio:rl.inicio,fim:rl.fim};});return {fixos:pf.concat(add),deltas:Object.assign({},(prev&&prev.deltas)||{})};});};
var dentPhone=function(pid){var x=(dents||[]).find(function(d){return d.id===pid;});return x?(x.phone||x.whatsapp||""):"";};
var diasResumo=function(rs){var s={};rs.forEach(function(r){(r.dias||[]).forEach(function(d){s[d]=1;});});var lab=[0,1,2,3,4,5,6].filter(function(d){return s[d];}).map(function(d){return DOW_LABELS[d];});return lab.join(", ");};
var hm=hhmmNow();var yest=new Date(now.getTime()-86400000);var act=[];
resolve(todayStr).forEach(function(e){var ini=e.inicio||"00:00",fim=e.fim||"23:59";if(fim>ini){if(hm>=ini&&hm<fim)act.push(e);}else{if(hm>=ini)act.push(e);}});
resolve(dstr(yest)).forEach(function(e){var ini=e.inicio||"00:00",fim=e.fim||"23:59";if(fim<=ini&&hm<fim)act.push(e);});
var actDent=act.filter(function(e){return e.tipo==="dent";});var actEq=act.filter(function(e){return e.tipo==="equipe";});
var meuProx=(function(){var did=user.dentistId,uid=user.id;for(var off=0;off<60;off++){var d=new Date(now.getTime()+off*86400000);var ds=dstr(d);var ents=resolve(ds);for(var k=0;k<ents.length;k++){var e=ents[k];var me=(e.tipo==="dent"&&did&&e.pid===did)||(e.tipo==="equipe"&&uid&&e.pid===uid);if(me){if(off===0&&(e.inicio||"")<=hm)continue;return {ds:ds,off:off,e:e};}}}return null;})();
var y=cur.y,m=cur.m;var first1=new Date(y,m,1);var startCol=first1.getDay();var ndays=new Date(y,m+1,0).getDate();var cells=[],i;
for(i=0;i<startCol;i++)cells.push(null);for(i=1;i<=ndays;i++)cells.push(i);while(cells.length%7!==0)cells.push(null);
var gapDias=0;for(i=1;i<=ndays;i++){var gds=y+"-"+pad(m+1)+"-"+pad(i);var hasD=resolve(gds).some(function(e){return e.tipo==="dent";});if(!hasD)gapDias++;}
var prevM=function(){setCur(function(c){var nm=c.m-1,ny=c.y;if(nm<0){nm=11;ny--;}return {y:ny,m:nm};});};
var nextM=function(){setCur(function(c){var nm=c.m+1,ny=c.y;if(nm>11){nm=0;ny++;}return {y:ny,m:nm};});};
var goHoje=function(){setCur({y:now.getFullYear(),m:now.getMonth()});setSelDate(todayStr);};
var lbl={fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:".4px",display:"block",marginBottom:6};
var inp={width:"100%",border:"1.5px solid "+G.border,borderRadius:9,padding:"11px 12px",fontSize:14,outline:"none",background:"#fff",boxSizing:"border-box"};
var lblm={fontSize:10.5,fontWeight:700,color:G.muted,letterSpacing:".5px",marginBottom:8};
var prow={display:"flex",alignItems:"center",gap:10,border:"1px solid "+G.border,background:"#fff",borderRadius:11,padding:"9px 11px",cursor:"pointer",width:"100%"};
return <div className="fi" style={{display:"flex",flexDirection:"column",gap:14}}>
<div style={{display:"flex",alignItems:"center",gap:10}}><h2 style={{fontFamily:"'Cormorant Garamond'",fontSize:26,margin:0}}>🗓️ Escala de Plantão</h2>{isAdmin&&<button onClick={function(){setListOpen(true);}} style={{marginLeft:"auto",border:"1.5px solid "+G.primary,background:"transparent",color:G.primary,borderRadius:9,padding:"7px 12px",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>👥 Escala fixa</button>}</div>
<div style={{background:"linear-gradient(135deg,#1E8449,#145A32)",borderRadius:14,padding:"14px 16px",color:"#fff",boxShadow:"0 4px 14px rgba(0,0,0,.15)"}}><div style={{fontSize:11,fontWeight:700,letterSpacing:"1px",opacity:.85,marginBottom:8}}>🟢 DE PLANTÃO AGORA</div>{act.length?<div style={{display:"flex",flexDirection:"column",gap:8}}>{actDent.map(function(e){var ph=dentPhone(e.pid);return <div key={e.key} style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14,fontWeight:700}}>{"🦷 "+(e.nome||"Dentista")}<span style={{fontWeight:500,opacity:.85,fontSize:12.5}}>{" · "+e.inicio+"–"+e.fim}</span></span>{ph&&<button onClick={function(){wa(ph,"Olá! Preciso falar com o dentista de plantão.");}} style={{marginLeft:"auto",background:"rgba(255,255,255,.18)",border:"none",color:"#fff",borderRadius:8,padding:"4px 11px",fontSize:12,fontWeight:700,cursor:"pointer"}}>💬 Chamar</button>}</div>;})}{actEq.length>0&&<div style={{fontSize:12.5,opacity:.9}}>{"👤 Equipe: "+actEq.map(function(e){return (e.nome||"Equipe")+" ("+e.inicio+"–"+e.fim+")";}).join(", ")}</div>}</div>:<div style={{fontSize:13,opacity:.9}}>Nenhum profissional escalado neste horário.</div>}</div>
{meuProx&&<div style={{background:G.accent,borderRadius:11,padding:"10px 14px",fontSize:13,color:G.primary,fontWeight:600}}>{"📌 Seu próximo plantão: "+(meuProx.off===0?"hoje":meuProx.off===1?"amanhã":(Number(meuProx.ds.split("-")[2])+" de "+MES[Number(meuProx.ds.split("-")[1])-1]))+" · "+meuProx.e.inicio+"–"+meuProx.e.fim}</div>}
<div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={prevM} style={{border:"1.5px solid "+G.border,background:"#fff",borderRadius:9,width:34,height:34,fontSize:16,fontWeight:700,color:G.primary,cursor:"pointer"}}>‹</button><div style={{fontFamily:"'Cormorant Garamond'",fontSize:20,fontWeight:700,color:G.text,minWidth:118}}>{MES[m]+" "+y}</div><button onClick={nextM} style={{border:"1.5px solid "+G.border,background:"#fff",borderRadius:9,width:34,height:34,fontSize:16,fontWeight:700,color:G.primary,cursor:"pointer"}}>›</button><button onClick={goHoje} style={{marginLeft:"auto",border:"none",background:G.accent,color:G.primary,borderRadius:8,padding:"7px 14px",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>Hoje</button></div>
{isAdmin&&gapDias>0&&<div style={{background:"#FDECEA",border:"1.5px solid "+G.red,borderRadius:10,padding:"8px 12px",fontSize:12.5,color:G.red,fontWeight:600}}>{"⚠️ "+gapDias+(gapDias===1?" dia sem dentista neste mês":" dias sem dentista neste mês")}</div>}
<div style={{background:G.card,border:"1px solid "+G.border,borderRadius:14,padding:"10px 10px 12px",boxShadow:"0 2px 10px rgba(30,132,73,.06)"}}>
<div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,marginBottom:4}}>{DOW_LABELS.map(function(d,idx){return <div key={idx} style={{textAlign:"center",fontSize:10.5,fontWeight:700,color:G.muted,padding:"2px 0"}}>{d[0]}</div>;})}</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>{cells.map(function(day,idx){if(day==null)return <div key={idx} style={{minHeight:64}}/>;var ds=y+"-"+pad(m+1)+"-"+pad(day);var ents=resolve(ds);var isToday=ds===todayStr;var edited=hasDelta(ds);return <button key={idx} onClick={function(){setSelDate(ds);}} style={{position:"relative",textAlign:"left",minHeight:64,border:"1.5px solid "+(isToday?G.primary:"#EEF2F0"),background:isToday?"#fff":((idx%7===0||idx%7===6)?"#FAFBFA":"#fff"),borderRadius:9,padding:"4px 4px 5px",cursor:"pointer",overflow:"hidden"}}><div style={{display:"flex",alignItems:"center",gap:3,marginBottom:2}}><span style={{fontSize:11.5,fontWeight:700,color:isToday?"#fff":G.text,background:isToday?G.primary:"transparent",borderRadius:5,minWidth:16,height:16,display:"inline-flex",alignItems:"center",justifyContent:"center",padding:isToday?"0 3px":0}}>{day}</span>{edited&&<span style={{marginLeft:"auto",width:6,height:6,borderRadius:"50%",background:G.red}}/>}</div><div style={{display:"flex",flexDirection:"column",gap:2}}>{ents.slice(0,2).map(function(e){var c=pColor(pKey(e.tipo,e.pid,e.nome));return <div key={e.key} style={{background:c,color:"#fff",borderRadius:4,fontSize:8.5,fontWeight:700,padding:"1px 3px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{first(e.nome)||(e.tipo==="dent"?"Dent":"Equipe")}</div>;})}{ents.length>2&&<div style={{fontSize:8.5,color:G.muted,fontWeight:700,paddingLeft:2}}>{"+"+(ents.length-2)}</div>}{ents.length===0&&<div style={{fontSize:9,color:"#C9D2CD",fontWeight:600,paddingLeft:2}}>—</div>}</div></button>;})}</div>
<div style={{display:"flex",flexWrap:"wrap",gap:"6px 12px",marginTop:10,paddingTop:8,borderTop:"1px solid "+G.border}}>{(function(){var seen={};var out=[];fixos.forEach(function(r){var k=pKey(r.tipo,r.pid,r.nome);if(!seen[k]){seen[k]=1;out.push({k:k,nome:r.nome,tipo:r.tipo});}});return out.slice(0,6).map(function(p){return <span key={p.k} style={{display:"inline-flex",alignItems:"center",gap:5,fontSize:10.5,color:G.text,fontWeight:600}}><span style={{width:10,height:10,borderRadius:3,background:pColor(p.k)}}/>{p.nome||(p.tipo==="dent"?"Dentista":"Equipe")}</span>;});})()}<span style={{display:"inline-flex",alignItems:"center",gap:5,fontSize:10.5,color:G.muted,fontWeight:600}}><span style={{width:7,height:7,borderRadius:"50%",background:G.red}}/>dia editado</span></div>
</div>
{selDate&&(function(){var ds=selDate;var ents=resolve(ds);var a=ds.split("-");var dObj=new Date(+a[0],+a[1]-1,+a[2]);var titulo=DOW_FULL[dObj.getDay()]+", "+Number(a[2])+" de "+MES[dObj.getMonth()];var dDent=ents.filter(function(e){return e.tipo==="dent";});var dEq=ents.filter(function(e){return e.tipo==="equipe";});var row=function(e){var c=pColor(pKey(e.tipo,e.pid,e.nome));return <div key={e.key} style={{background:G.bg,borderRadius:10,padding:"9px 10px",display:"flex",alignItems:"center",gap:8}}><span style={{width:26,height:26,borderRadius:"50%",background:c,color:"#fff",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,flexShrink:0}}>{(first(e.nome)[0]||"?").toUpperCase()}</span><div style={{minWidth:0,flex:"1 1 auto"}}><div style={{fontSize:13,fontWeight:700,color:G.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{e.nome||(e.tipo==="dent"?"Dentista":"Equipe")}</div><div style={{fontSize:10,color:G.muted,fontWeight:600}}>{e.fixo?"fixo":"avulso"}</div></div>{isAdmin?<div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}><input type="time" value={e.inicio} onChange={function(ev){var v=ev.target.value;if(e.fixo)setFixoHr(ds,e.ruleId,{inicio:v,fim:e.fim});else setExtraHr(ds,e.addId,{inicio:v});}} style={{border:"1.5px solid "+G.border,borderRadius:7,padding:"5px 4px",fontSize:11.5,outline:"none",width:70,boxSizing:"border-box"}}/><span style={{color:G.muted,fontSize:11}}>–</span><input type="time" value={e.fim} onChange={function(ev){var v=ev.target.value;if(e.fixo)setFixoHr(ds,e.ruleId,{inicio:e.inicio,fim:v});else setExtraHr(ds,e.addId,{fim:v});}} style={{border:"1.5px solid "+G.border,borderRadius:7,padding:"5px 4px",fontSize:11.5,outline:"none",width:70,boxSizing:"border-box"}}/><button onClick={function(){if(e.fixo)delFixo(ds,e.ruleId);else delExtra(ds,e.addId);}} style={{border:"none",background:"#FDECEA",color:G.red,borderRadius:7,padding:"5px 7px",fontSize:13,cursor:"pointer",fontWeight:700}}>🗑</button></div>:<span style={{marginLeft:"auto",fontSize:12.5,color:G.muted,fontWeight:600}}>{(e.inicio||"")+"–"+(e.fim||"")}</span>}</div>;};return <Modal open={true} close={function(){setSelDate(null);}} title={titulo} ch={<div style={{display:"flex",flexDirection:"column",gap:14}}>{hasDelta(ds)&&<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{background:"#FCEBE7",color:G.red,borderRadius:7,padding:"3px 9px",fontSize:11,fontWeight:700}}>Exceção neste dia</span><button onClick={function(){restoreDay(ds);}} style={{marginLeft:"auto",border:"none",background:"transparent",color:G.primary,fontSize:12,fontWeight:700,cursor:"pointer",textDecoration:"underline"}}>Restaurar escala fixa</button></div>}<div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,marginBottom:6,letterSpacing:".4px"}}>DENTISTAS</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{dDent.length?dDent.map(row):<div style={{fontSize:12.5,color:G.red,fontWeight:600}}>Sem dentista neste dia</div>}</div>{isAdmin&&<button onClick={function(){setAf({pid:"",nome:"",inicio:"08:00",fim:"18:00"});setDayAdd({tipo:"dent"});}} style={{marginTop:8,border:"1.5px dashed "+G.primary,background:"transparent",color:G.primary,borderRadius:9,padding:"9px",fontSize:12.5,fontWeight:700,cursor:"pointer",width:"100%"}}>+ Adicionar dentista</button>}</div><div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,marginBottom:6,letterSpacing:".4px"}}>SECRETÁRIAS / RECEPÇÃO</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{dEq.length?dEq.map(row):<div style={{fontSize:12.5,color:G.muted}}>—</div>}</div>{isAdmin&&<button onClick={function(){setAf({pid:"",nome:"",inicio:"08:00",fim:"17:00"});setDayAdd({tipo:"equipe"});}} style={{marginTop:8,border:"1.5px dashed "+G.primary,background:"transparent",color:G.primary,borderRadius:9,padding:"9px",fontSize:12.5,fontWeight:700,cursor:"pointer",width:"100%"}}>+ Adicionar secretária</button>}</div>{isAdmin&&<div style={{fontSize:11,color:G.muted,lineHeight:1.4}}>O que você mexer aqui vale só para este dia. Para mudar todo mês, use “Escala fixa”.</div>}</div>}/>;})()}
{dayAdd&&(function(){var tipo=dayAdd.tipo;var pool=tipo==="dent"?(dents||[]):(users||[]);var confirm=function(){var isOutro=af.pid==="__outro__";var pid=(af.pid&&!isOutro)?Number(af.pid):null;var nome=pid?(((pool.find(function(x){return x.id===pid;})||{}).name)||""):((af.nome||"").trim());if(!pid&&!nome)return;addDay(selDate,{tipo:tipo,pid:pid,nome:nome,inicio:af.inicio,fim:af.fim});setDayAdd(null);};return <Modal open={true} close={function(){setDayAdd(null);}} title={tipo==="dent"?"Adicionar dentista":"Adicionar à recepção"} ch={<div style={{display:"flex",flexDirection:"column",gap:12}}><div><label style={lbl}>{tipo==="dent"?"Dentista":"Pessoa"}</label><select value={af.pid} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{pid:v,nome:""});});}} style={inp}><option value="">— selecionar —</option>{pool.map(function(x){return <option key={x.id} value={x.id}>{x.name}</option>;})}<option value="__outro__">Outro (digitar nome)</option></select></div>{af.pid==="__outro__"&&<div><label style={lbl}>Nome</label><input value={af.nome} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{nome:v});});}} style={inp}/></div>}<div style={{display:"flex",gap:10}}><div style={{flex:1}}><label style={lbl}>ENTRA</label><input type="time" value={af.inicio} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{inicio:v});});}} style={inp}/></div><div style={{flex:1}}><label style={lbl}>SAI</label><input type="time" value={af.fim} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{fim:v});});}} style={inp}/></div></div><div style={{fontSize:11.5,color:G.muted,lineHeight:1.4}}>Plantão que vira o dia: a saída pode ser menor que a entrada (ex.: 19:00 → 07:00).</div><Btn ch="Adicionar" onClick={confirm}/></div>}/>;})()}
{listOpen&&<Modal open={true} close={function(){setListOpen(false);}} title="Escala fixa · profissionais" ch={<div style={{display:"flex",flexDirection:"column",gap:14}}><div style={{fontSize:12,color:G.muted,lineHeight:1.45}}>Escolha quem trabalha fixo e em quais dias da semana. Vale para todos os meses, automático.</div><div><div style={lblm}>DENTISTAS</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{(dents||[]).map(function(x){var rs=fixos.filter(function(r){return r.tipo==="dent"&&r.pid===x.id;});return <button key={x.id} onClick={function(){setEdit({tipo:"dent",pid:x.id,nome:x.name});}} style={prow}><span style={{width:28,height:28,borderRadius:"50%",background:pColor(pKey("dent",x.id,x.name)),color:"#fff",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:12.5,fontWeight:700,flexShrink:0}}>{(x.name[0]||"?").toUpperCase()}</span><div style={{textAlign:"left",minWidth:0}}><div style={{fontSize:13.5,fontWeight:700,color:G.text}}>{x.name}</div><div style={{fontSize:10.5,color:rs.length?G.primary:G.muted,fontWeight:600}}>{rs.length?diasResumo(rs):"sem dias fixos"}</div></div><span style={{marginLeft:"auto",color:G.muted,fontSize:18}}>›</span></button>;})}</div></div><div><div style={lblm}>SECRETÁRIAS / RECEPÇÃO</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{(users||[]).map(function(x){var rs=fixos.filter(function(r){return r.tipo==="equipe"&&r.pid===x.id;});return <button key={x.id} onClick={function(){setEdit({tipo:"equipe",pid:x.id,nome:x.name});}} style={prow}><span style={{width:28,height:28,borderRadius:"50%",background:pColor(pKey("equipe",x.id,x.name)),color:"#fff",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:12.5,fontWeight:700,flexShrink:0}}>{(x.name[0]||"?").toUpperCase()}</span><div style={{textAlign:"left",minWidth:0}}><div style={{fontSize:13.5,fontWeight:700,color:G.text}}>{x.name}</div><div style={{fontSize:10.5,color:rs.length?G.primary:G.muted,fontWeight:600}}>{rs.length?diasResumo(rs):"sem dias fixos"}</div></div><span style={{marginLeft:"auto",color:G.muted,fontSize:18}}>›</span></button>;})}</div></div></div>}/>}
{edit&&<PersonEditor target={edit} fixos={fixos} deltas={deltas} onClose={function(){setEdit(null);}} onSaveRules={saveFixos} addDay={addDay} delExtra={delExtra} pColor={pColor} pKey={pKey}/>}
</div>;}
...
const [escala,setEscala]=useState(function(){try{var v3=JSON.parse(localStorage.getItem("orbe_escala_v3")||"null");if(v3&&v3.fixos)return {fixos:v3.fixos||[],deltas:v3.deltas||{}};}catch(e){}try{var v2=JSON.parse(localStorage.getItem("orbe_escala_v2")||"null");if(v2&&typeof v2==="object"){var fx=[];Object.keys(v2).forEach(function(k){var wd=Number(k);if(isNaN(wd))return;(v2[k]||[]).forEach(function(e){fx.push({id:Date.now()+fx.length,tipo:e.tipo,pid:e.pid,nome:e.nome,dias:[wd],inicio:e.inicio,fim:e.fim});});});if(fx.length)return {fixos:fx,deltas:{}};}}catch(e){}return {fixos:[],deltas:{}};});
useEffect(function(){try{localStorage.setItem("orbe_escala_v3",JSON.stringify(escala));}catch(e){}},[escala]);
