...
,async fetchPortal(token){var r=await orbeApi("fetchPortal",{token:token});return r.ok?((r.j&&r.j.portal)||null):null;}
,async sendPortalAction(token,payload){var r=await orbeApi("sendPortalAction",{token:token,payload:payload});return r.ok?{ok:true}:{ok:false,msg:(r.j&&r.j.msg)||"Erro"};}
...
// ════════════════════════════════════════════════════════════════
// PORTAL DO PACIENTE — link pessoal por paciente (sem login, sem app)
// ════════════════════════════════════════════════════════════════
const PORTAL_SECTIONS=[
  ["proxima","Próxima consulta","Data, horário e profissional"],
  ["tratamento","Tratamento e orçamento","Itens em andamento e orçamento"],
  ["fotos","Fotos antes / depois","Imagens marcadas como antes/depois"],
  ["financeiro","Pendência financeira","Saldo em aberto do paciente"],
  ["presenca","Confirmar presença","Botão para o paciente confirmar a consulta"],
  ["clinica","Dados da clínica","Nome, endereço, telefone e WhatsApp"]
];
const PORTAL_DEFAULT_OPTS={proxima:true,tratamento:true,fotos:true,financeiro:true,presenca:true,clinica:true};

function genToken(){
  try{
    var a=new Uint8Array(20);(window.crypto||window.msCrypto).getRandomValues(a);
    var s="";for(var i=0;i<a.length;i++){s+=("0"+a[i].toString(16)).slice(-2);}
    return s.slice(0,28);
  }catch(e){
    var t="";var c="abcdefghijklmnopqrstuvwxyz0123456789";
    for(var j=0;j<28;j++)t+=c[Math.floor(Math.random()*c.length)];
    return t;
  }
}

// Domínio base do link. Usa CLINICA_LIVE.appUrl se preenchido; senão a URL atual.
function appBase(){
  try{
    var u=((CLINICA_LIVE&&CLINICA_LIVE.appUrl)||"").trim();
    if(u){if(!/^https?:\/\//i.test(u))u="https://"+u;return u.replace(/[?#].*$/,"").replace(/\/+$/,"");}
  }catch(e){}
  return window.location.origin+window.location.pathname;
}
function portalLink(token){return appBase()+"?portal="+encodeURIComponent(token||"");}

// Recalcula o resumo público do paciente. Só inclui as seções LIGADAS (o que está
// desligado não é gravado — não vaza). Não inclui updatedAt (esse é só na gravação).
function buildPortalSnapshot(pat,opts,ctx){
  var appts=ctx.appts||[],treats=ctx.treats||[],budgets=ctx.budgets||[],dents=ctx.dents||[],clinica=ctx.clinica||{},td=ctx.today;
  var o=opts||PORTAL_DEFAULT_OPTS;
  var snap={name:pat.name||""};
  var prox=appts.filter(function(a){return a.patientId===pat.id&&a.date>=td&&a.status!=="cancelled"&&a.status!=="missed"&&a.status!=="rescheduled"&&a.status!=="done"&&a.status!=="blocked";})
    .sort(function(a,b){return (a.date+(a.time||"")).localeCompare(b.date+(b.time||""));})[0];
  if(o.clinica){snap.clinica={nome:clinica.nome||"",endereco:clinica.endereco||"",telefone:clinica.telefone||"",whatsapp:clinica.whatsapp||""};}
  if(o.proxima&&prox){
    var dp=dents.find(function(x){return x.id===prox.dentistId;});
    snap.proxima={date:prox.date,time:prox.time||"",procedure:prox.procedureCustom||prox.procedure||"Consulta",dentista:dp?dp.name:"",status:prox.status};
  }
  if(o.presenca&&prox){snap.presenca={apptId:prox.id,date:prox.date,time:prox.time||"",confirmado:prox.status==="confirmed"};}
  if(o.tratamento){
    var at=treats.filter(function(t){return t.patientId===pat.id&&(t.items||[]).some(function(it){return !(it.done||it.paid);});})
      .sort(function(a,b){return (b.start||"").localeCompare(a.start||"");})[0];
    if(at){snap.tratamento={name:at.name||"Tratamento",items:(at.items||[]).map(function(it){return {desc:it.desc||"",feito:!!(it.done||it.paid)};})};}
    var bd=budgets.filter(function(b){return b.patientId===pat.id;}).sort(function(a,b){return (b.date||"").localeCompare(a.date||"");})[0];
    if(bd){
      var sub=(bd.items||[]).reduce(function(s,it){return s+(Number(it.v)||0);},0);
      snap.orcamento={date:bd.date||"",status:bd.status||"",total:sub-(Number(bd.disc)||0),items:(bd.items||[]).map(function(it){return {d:it.d||"",v:Number(it.v)||0};})};
    }
  }
  if(o.financeiro){
    var pend=0;
    treats.filter(function(t){return t.patientId===pat.id;}).forEach(function(t){(t.items||[]).forEach(function(it){if(!it.paid)pend+=(Number(it.value)||0);});});
    snap.financeiro={pendente:pend};
  }
  if(o.fotos){
    var fotos=(pat.imagens||[]).filter(function(im){return im.cat==="antesdepois"&&im.url;})
      .sort(function(a,b){return (b.date||"").localeCompare(a.date||"");}).slice(0,8)
      .map(function(im){return {url:im.url,date:im.date||""};});
    if(fotos.length)snap.fotos=fotos;
  }
  return snap;
}

function PortalSwitch({on,onToggle,label,desc}){
  return <button type="button" onClick={onToggle} style={{display:"flex",alignItems:"center",gap:11,width:"100%",textAlign:"left",border:"1.5px solid "+(on?G.primary:G.border),background:on?G.accent:G.card,borderRadius:11,padding:"10px 12px",cursor:"pointer"}}>
    <span style={{width:38,height:22,borderRadius:12,background:on?G.primary:G.border,position:"relative",flexShrink:0,transition:"background .15s"}}>
    ...
    <span style={{flex:1}}>
      <span style={{display:"block",fontWeight:700,fontSize:13,color:G.text}}>{label}</span>
      <span style={{display:"block",fontSize:11,color:G.muted,marginTop:1}}>{desc}</span>
    </span>
  </button>;
}
...
function PortalModal({pat,setPats,onClose}){
  const live=pat;
  const token=live.portalToken||"";
  const opts=live.portalOpts||PORTAL_DEFAULT_OPTS;
  const [copied,setCopied]=useState(false);
  const setOpt=function(key){
    setPats(function(prev){return prev.map(function(p){return p.id===pat.id?Object.assign({},p,{portalOpts:Object.assign({},p.portalOpts||PORTAL_DEFAULT_OPTS,{[key]:!(p.portalOpts||PORTAL_DEFAULT_OPTS)[key]})}):p;});});
  };
  const gerar=function(novo){
    setPats(function(prev){return prev.map(function(p){return p.id===pat.id?Object.assign({},p,{portalToken:(novo||!p.portalToken)?genToken():p.portalToken,portalOpts:p.portalOpts||PORTAL_DEFAULT_OPTS}):p;});});
  };
  const link=token?portalLink(token):"";
  const copiar=function(){if(!link)return;try{navigator.clipboard.writeText(link);}catch(e){}setCopied(true);setTimeout(function(){setCopied(false);},1500);};
  const enviarWA=function(){if(!link)return;var msg="Olá, "+pat.name+"! 😊\n\nEste é o seu portal pessoal da "+(CLINICA_LIVE.nome||"clínica")+". Pelo link você vê sua próxima consulta, tratamento e mais — sem precisar instalar nada:\n\n"+link;wa(pat.phone,msg);};
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:16,overflowY:"auto"}}>
    <div style={{background:G.card,borderRadius:18,width:"100%",maxWidth:460,margin:"16px auto",boxShadow:"0 8px 32px rgba(0,0,0,.2)"}}>
      <div style={{background:G.primary,borderRadius:"18px 18px 0 0",padding:"14px 18px",display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:20}}>{"🔗"}</span>
        <div style={{flex:1}}><div style={{fontWeight:700,color:"#fff",fontSize:15}}>Portal do paciente</div><div style={{fontSize:11,color:"rgba(255,255,255,.85)"}}>{pat.name}</div></div>
      ...
      <div style={{padding:18,display:"flex",flexDirection:"column",gap:12}}>
        <div style={{fontSize:12.5,color:G.muted,lineHeight:1.45}}>Escolha o que este link mostra. O que estiver desligado não é gravado e não aparece para o paciente.</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {PORTAL_SECTIONS.map(function(s){return <PortalSwitch key={s[0]} on={!!opts[s[0]]} onToggle={function(){setOpt(s[0]);}} label={s[1]} desc={s[2]}/>;})}
        </div>
        {!token
          ?<button onClick={function(){gerar(false);}} style={{background:G.primary,color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:15,fontWeight:700,cursor:"pointer"}}>{"🔗 Gerar link do portal"}</button>
          :<div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{background:G.accent,borderRadius:10,padding:"10px 12px",fontSize:11.5,color:G.primary,wordBreak:"break-all",fontWeight:600}}>{link}</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <button onClick={enviarWA} style={{flex:1,minWidth:140,background:"#25D366",color:"#fff",border:"none",borderRadius:11,padding:"12px",fontSize:14,fontWeight:700,cursor:"pointer"}}>{"📱 Enviar por WhatsApp"}</button>
              <button onClick={copiar} style={{flex:1,minWidth:110,background:G.card,color:G.primary,border:"1.5px solid "+G.primary,borderRadius:11,padding:"12px",fontSize:14,fontWeight:700,cursor:"pointer"}}>{copied?"✓ Copiado":"📋 Copiar link"}</button>
            </div>
            <button onClick={function(){if(window.confirm("Gerar um link novo invalida o link anterior. Continuar?"))gerar(true);}} style={{background:"transparent",color:G.muted,border:"none",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"center",padding:"4px"}}>{"↻ Gerar link novo (invalida o anterior)"}</button>
          </div>}
        <div style={{fontSize:11,color:G.muted,lineHeight:1.4}}>O link não expira. Ele começa a funcionar assim que o paciente é salvo na nuvem (poucos segundos após gerar).</div>
      </div>
    </div>
  </div>;
}

// ── ROTA PÚBLICA: a tela que o paciente abre pelo link ──
function PatientPortal({token}){
  const [state,setState]=useState("load"); // load | ok | none | err
  const [p,setP]=useState(null);
  const [conf,setConf]=useState(""); // "" | sending | done | err
  useEffect(function(){
    var alive=true;
    if(!SUPA_URL){setState("none");return;}
    supabase.fetchPortal(token).then(function(portal){
      if(!alive)return;
      if(portal){setP(portal);setState("ok");}else{setState("none");}
    }).catch(function(){if(alive)setState("err");});
    return function(){alive=false;};
  },[token]);
  const confirmar=function(){
    if(!p||!p.presenca)return;
    setConf("sending");
    supabase.sendPortalAction(token,{type:"confirm",apptId:p.presenca.apptId,date:p.presenca.date}).then(function(r){
      setConf(r&&r.ok?"done":"err");
    }).catch(function(){setConf("err");});
  };
  const wrap=function(children){return <div style={{minHeight:"100vh",background:G.bg,display:"flex",justifyContent:"center",padding:"16px"}}><div style={{width:"100%",maxWidth:440}}>{children}</div></div>;};
  if(state==="load")return wrap(<div style={{textAlign:"center",padding:"60px 0",color:G.muted,fontSize:14}}>Carregando…</div>);
  if(state!=="ok"||!p)return wrap(<div style={{background:G.card,borderRadius:16,padding:"40px 24px",textAlign:"center",marginTop:40}}>
    <div style={{fontSize:40,marginBottom:10}}>{"🔗"}</div>
    <div style={{fontWeight:700,fontSize:16,color:G.text,marginBottom:6}}>Link não encontrado</div>
    <div style={{fontSize:13,color:G.muted,lineHeight:1.5}}>Este link pode ter sido atualizado. Peça um novo link à clínica.</div>
  </div>);
  const card={background:G.card,borderRadius:16,padding:"16px 18px",boxShadow:"0 2px 10px rgba(0,0,0,.05)"};
  const titulo=function(t){return <div style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:".5px",marginBottom:8}}>{t}</div>;};
  return wrap(<div style={{display:"flex",flexDirection:"column",gap:14,paddingBottom:30}}>
    <div style={{textAlign:"center",padding:"22px 0 6px"}}>
      <div style={{fontFamily:"'Cormorant Garamond'",fontSize:26,color:G.primary,lineHeight:1.1}}>{(p.clinica&&p.clinica.nome)||"Seu portal"}</div>
      <div style={{fontSize:14,color:G.text,marginTop:4}}>{"Olá, "+(p.name||"")+" 👋"}</div>
    </div>

    {p.proxima&&<div style={card}>
      {titulo("📅 Próxima consulta")}
      <div style={{fontFamily:"'Cormorant Garamond'",fontSize:22,color:G.primary,fontWeight:700,lineHeight:1.1}}>{fmt(p.proxima.date)+(p.proxima.time?" às "+p.proxima.time:"")}</div>
      <div style={{fontSize:13,color:G.text,marginTop:4}}>{(p.proxima.procedure||"Consulta")+(p.proxima.dentista?" · "+p.proxima.dentista:"")}</div>
      {p.proxima.status&&<div style={{display:"inline-block",marginTop:8,fontSize:11.5,fontWeight:700,color:G.primary,background:G.accent,borderRadius:8,padding:"3px 9px"}}>{SL[p.proxima.status]||p.proxima.status}</div>}
    </div>}

    {p.presenca&&<div style={card}>
      {titulo("✅ Confirmação de presença")}
      {p.presenca.confirmado||conf==="done"
        ?<div style={{fontSize:14,color:G.success,fontWeight:700}}>{"✓ Presença confirmada. Até breve!"}</div>
        :<div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{fontSize:13,color:G.text}}>{"Você confirma a consulta de "+fmt(p.presenca.date)+(p.presenca.time?" às "+p.presenca.time:"")+"?"}</div>
          <button onClick={confirmar} disabled={conf==="sending"} style={{background:conf==="sending"?G.muted:G.success,color:"#fff",border:"none",borderRadius:11,padding:"12px",fontSize:14,fontWeight:700,cursor:conf==="sending"?"default":"pointer"}}>{conf==="sending"?"Enviando…":"✅ Confirmar presença"}</button>
          {conf==="err"&&<div style={{fontSize:12,color:G.red}}>Não foi possível confirmar agora. Tente de novo em instantes.</div>}
        </div>}
    </div>}

    {p.tratamento&&<div style={card}>
      {titulo("🦷 Seu tratamento")}
      <div style={{fontWeight:700,fontSize:14,color:G.text,marginBottom:6}}>{p.tratamento.name}</div>
      {(p.tratamento.items||[]).map(function(it,i){return <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:"1px solid "+G.border,fontSize:13,color:G.text}}>
        <span style={{color:it.feito?G.success:G.muted}}>{it.feito?"✓":"○"}</span>
        <span style={{flex:1,textDecoration:it.feito?"line-through":"none",opacity:it.feito?.6:1}}>{it.desc}</span>
      </div>;})}
    </div>}

    {p.orcamento&&<div style={card}>
      {titulo("💬 Orçamento")}
      {(p.orcamento.items||[]).map(function(it,i){return <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:13,color:G.text,padding:"4px 0"}}><span>{it.d}</span><span style={{fontWeight:600}}>{cur(it.v)}</span></div>;})}
      <div style={{display:"flex",justifyContent:"space-between",marginTop:8,paddingTop:8,borderTop:"1.5px solid "+G.border,fontSize:14,fontWeight:700,color:G.primary}}><span>Total</span><span>{cur(p.orcamento.total)}</span></div>
    </div>}

    {p.financeiro&&p.financeiro.pendente>0&&<div style={card}>
      {titulo("💰 Financeiro")}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:13,color:G.text}}>Saldo em aberto</span>
        <span style={{fontSize:18,fontWeight:700,color:G.red,fontFamily:"'Cormorant Garamond'"}}>{cur(p.financeiro.pendente)}</span>
      </div>
    </div>}

    {p.fotos&&p.fotos.length>0&&<div style={card}>
      {titulo("✨ Antes e depois")}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {p.fotos.map(function(f,i){return <img key={i} src={f.url} alt="" style={{width:"100%",borderRadius:10,objectFit:"cover",aspectRatio:"1/1"}}/>;})}
      </div>
    </div>}

    {p.clinica&&<div style={card}>
      {titulo("🏥 "+(p.clinica.nome||"A clínica"))}
      {p.clinica.endereco&&<div style={{fontSize:13,color:G.text,marginBottom:4}}>{"📍 "+p.clinica.endereco}</div>}
      {p.clinica.telefone&&<div style={{fontSize:13,color:G.text,marginBottom:4}}>{"☎️ "+p.clinica.telefone}</div>}
      {p.clinica.whatsapp&&<button onClick={function(){var n=(p.clinica.whatsapp||"").replace(/\D/g,"");window.open("https://wa.me/"+(n.startsWith("55")?n:"55"+n),"_blank");}} style={{marginTop:6,background:"#25D366",color:"#fff",border:"none",borderRadius:10,padding:"10px 14px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{"💬 Falar no WhatsApp"}</button>}
    </div>}

    <div style={{textAlign:"center",fontSize:11,color:G.muted,marginTop:6}}>{p.updatedAt?("Atualizado em "+new Date(p.updatedAt).toLocaleDateString("pt-BR")):""}</div>
  </div>);
}
...
const [pf,setPf]=useState({...pat});const [showWAanam,setShowWAanam]=useState(false);const [showIARX,setShowIARX]=useState(false);const [fillAnam,setFillAnam]=useState(false);const [buscaMsg,setBuscaMsg]=useState("");const [showPortal,setShowPortal]=useState(false);
...
<div style={{padding:22}}>
  ...
  {tab==="ficha"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      {!editMode?<div style={{display:"flex",gap:5}}><Btn ch="🔗 Portal" v="g" sm onClick={function(){setShowPortal(true);}}/><Btn ch="📋 WA" v="g" sm onClick={function(){setShowWAanam(true);}}/><Btn ch="✏️ Editar" v="g" sm onClick={()=>setEditMode(true)}/></div>:<div style={{display:"flex",gap:8}}><Btn ch="💾 Salvar" sm onClick={savePat}/><Btn ch="Cancelar" v="g" sm onClick={()=>{setPf({...pat});setEditMode(false);}}/></div>}
    </div>
...
<div key={slot} style={{display:"flex",alignItems:"center",gap:5,padding:"1px 6px",borderRadius:5,background:"#F3E5F5",border:"1px solid #CE93D8"}}>
...
<div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
<span style={{fontWeight:700,fontSize:12,color:isPartial?G.red:G.text}}>{isPartial?a.patientName:(p&&p.name)}</span>
...
{p&&anamFalta(p)&&<div style={{display:"flex",marginTop:2}}><span style={{fontSize:9.5,background:"#D81B60",color:"#fff",borderRadius:5,padding:"2px 8px",fontWeight:800,letterSpacing:".3px"}}>{"⚠ ANAMNESE NÃO CADASTRADA"}</span></div>}
...
{dm&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
  <div style={{background:G.card,borderRadius:16,width:"100%",maxWidth:560,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 16px 48px rgba(0,0,0,.22)"}}>
    ...
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4}}><label style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:".4px"}}>Intervalo da agenda</label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[["",((df.specialty||"").toLowerCase().indexOf("orto")>=0?"Padrão (20m)":"Padrão (30m)")],[15,"15m"],[20,"20m"],[30,"30m"],[45,"45m"],[60,"60m"]].map(function(o){var v=o[0];var lab=o[1];var cur=df.slotMin?Number(df.slotMin):"";var on=cur===v;return <button key={String(v)} type="button" onClick={function(){upDf("slotMin")(v);}} style={{border:"2px solid "+(on?G.primary:G.border),background:on?G.primary:"#fff",color:on?"#fff":G.text,borderRadius:9,padding:"7px 12px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{lab}</button>;})}</div><div style={{fontSize:11,color:G.muted,marginTop:2,lineHeight:1.4}}>Tempo de cada horário na agenda deste dentista. "Padrão" usa 20 min para Orto e 30 min para os demais.</div></div>
      <div style={{display:"flex",gap:9,justifyContent:"flex-end",paddingTop:12,borderTop:"1px solid "+G.border}}>
...
<div style={{fontSize:16,fontWeight:700,color:G.primary,marginBottom:5}}>{c[0]+" "+c[1]}</div>
...
function Escala({dents,users,user,escala,setEscala}){var isAdmin=user.level>=3;var [selDia,setSelDia]=useState(new Date().getDay());var [addOpen,setAddOpen]=useState(null);var [af,setAf]=useState({pid:"",nome:"",inicio:"08:00",fim:"18:00"});var [copyOpen,setCopyOpen]=useState(false);var [copySel,setCopySel]=useState({});var dOrder=[1,2,3,4,5,6,0];var dayEntries=function(dia){return (escala&&escala[dia])||[];};var dentPhone=function(pid){var x=(dents||[]).find(function(d){return d.id===pid;});return x?(x.phone||x.whatsapp||""):"";};var setDay=function(dia,arr){setEscala(function(prev){var nx=Object.assign({},prev);nx[dia]=arr;return nx;});};var addEntry=function(dia,e){setEscala(function(prev){var nx=Object.assign({},prev);nx[dia]=(nx[dia]||[]).concat([e]);return nx;});};var updEntry=function(dia,id,patch){setEscala(function(prev){var nx=Object.assign({},prev);nx[dia]=(nx[dia]||[]).map(function(x){return x.id===id?Object.assign({},x,patch):x;});return nx;});};var delEntry=function(dia,id){setEscala(function(prev){var nx=Object.assign({},prev);nx[dia]=(nx[dia]||[]).filter(function(x){return x.id!==id;});return nx;});};var act=plantaoAgora(escala);var actDent=act.filter(function(e){return e.tipo==="dent";});var actEq=act.filter(function(e){return e.tipo==="equipe";});var gaps=[];if(isAdmin){dOrder.forEach(function(dia){var hasD=dayEntries(dia).some(function(e){return e.tipo==="dent";});if(!hasD)gaps.push(DOW_LABELS[dia]);});}var meuProx=(function(){var did=user.dentistId,uid=user.id,nd=new Date().getDay(),hm=hhmmNow();for(var off=0;off<7;off++){var dia=(nd+off)%7;var ents=dayEntries(dia).slice().sort(function(a,b){return (a.inicio||"").localeCompare(b.inicio||"");});for(var k=0;k<ents.length;k++){var e=ents[k];var me=(e.tipo==="dent"&&did&&e.pid===did)||(e.tipo==="equipe"&&uid&&e.pid===uid);if(me){if(off===0&&(e.inicio||"")<=hm){continue;}return {dia:dia,e:e,off:off};}}}return null;})();var selEnts=dayEntries(selDia).slice().sort(function(a,b){return (a.inicio||"").localeCompare(b.inicio||"");});var selDent=selEnts.filter(function(e){return e.tipo==="dent";});var selEq=selEnts.filter(function(e){return e.tipo==="equipe";});var openAdd=function(tipo){setAf({pid:"",nome:"",inicio:"08:00",fim:"18:00"});setAddOpen(tipo);};var confirmAdd=function(){var tipo=addOpen;var isOutro=af.pid==="__outro__";var pid=(af.pid&&!isOutro)?Number(af.pid):null;var nome=pid?((((tipo==="dent")?(dents||[]):(users||[])).find(function(x){return x.id===pid;})||{}).name||""):((af.nome||"").trim());if(!pid&&!nome){return;}addEntry(selDia,{id:Date.now()+Math.floor(Math.random()*100000),tipo:tipo,pid:pid,nome:nome,inicio:af.inicio,fim:af.fim});setAddOpen(null);setAf({pid:"",nome:"",inicio:"08:00",fim:"18:00"});};var applyCopy=function(){var src=dayEntries(selDia);Object.keys(copySel).forEach(function(d){if(copySel[d]){var dia=Number(d);var copy=src.map(function(e){return Object.assign({},e,{id:Date.now()+Math.floor(Math.random()*1000000)});});setDay(dia,copy);}});setCopyOpen(false);setCopySel({});};var entryCard=function(e){var nome=e.nome||(e.tipo==="dent"?"Dentista":"Equipe");return <div key={e.id} style={{background:G.bg,borderRadius:10,padding:"10px 12px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontSize:13.5,fontWeight:700,color:G.text}}>{(e.tipo==="dent"?"🦷 ":"👤 ")+nome}</span>{isAdmin?<div style={{display:"flex",alignItems:"center",gap:5,marginLeft:"auto"}}><input type="time" value={e.inicio} onChange={function(ev){updEntry(selDia,e.id,{inicio:ev.target.value});}} style={{border:"1.5px solid "+G.border,borderRadius:7,padding:"5px 7px",fontSize:12.5,outline:"none"}}/><span style={{color:G.muted}}>–</span><input type="time" value={e.fim} onChange={function(ev){updEntry(selDia,e.id,{fim:ev.target.value});}} style={{border:"1.5px solid "+G.border,borderRadius:7,padding:"5px 7px",fontSize:12.5,outline:"none"}}/><button onClick={function(){delEntry(selDia,e.id);}} style={{border:"none",background:"#FDECEA",color:G.red,borderRadius:7,padding:"5px 9px",fontSize:13,cursor:"pointer",fontWeight:700}}>🗑</button></div>:<span style={{marginLeft:"auto",fontSize:13,color:G.muted,fontWeight:600}}>{(e.inicio||"")+"–"+(e.fim||"")}</span>}</div>;};return <div className="fi" style={{display:"flex",flexDirection:"column",gap:14}}><h2 style={{fontFamily:"'Cormorant Garamond'",fontSize:26}}>🗓️ Escala de Plantão</h2><div style={{background:"linear-gradient(135deg,#1E8449,#145A32)",borderRadius:14,padding:"14px 16px",color:"#fff",boxShadow:"0 4px 14px rgba(0,0,0,.15)"}}><div style={{fontSize:11,fontWeight:700,letterSpacing:"1px",opacity:.85,marginBottom:8}}>🟢 DE PLANTÃO AGORA</div>{(act.length)?<div style={{display:"flex",flexDirection:"column",gap:8}}>{actDent.map(function(e){var ph=dentPhone(e.pid);return <div key={e.id} style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14,fontWeight:700}}>{"🦷 "+(e.nome||"Dentista")}<span style={{fontWeight:500,opacity:.85,fontSize:12.5}}>{" · "+e.inicio+"–"+e.fim}</span></span>{ph&&<button onClick={function(){wa(ph,"Olá! Preciso falar com o dentista de plantão.");}} style={{marginLeft:"auto",background:"rgba(255,255,255,.18)",border:"none",color:"#fff",borderRadius:8,padding:"4px 11px",fontSize:12,fontWeight:700,cursor:"pointer"}}>💬 Chamar</button>}</div>;})}{actEq.length>0&&<div style={{fontSize:12.5,opacity:.9}}>{"👤 Equipe: "+actEq.map(function(e){return (e.nome||"Equipe")+" ("+e.inicio+"–"+e.fim+")";}).join(", ")}</div>}</div>:<div style={{fontSize:13,opacity:.9}}>Nenhum profissional escalado neste horário.</div>}</div>{meuProx&&<div style={{background:G.accent,borderRadius:11,padding:"10px 14px",fontSize:13,color:G.primary,fontWeight:600}}>{"📌 Seu próximo plantão: "+(meuProx.off===0?"hoje":meuProx.off===1?"amanhã":DOW_FULL[meuProx.dia])+" · "+meuProx.e.inicio+"–"+meuProx.e.fim}</div>}{isAdmin&&gaps.length>0&&<div style={{background:"#FDECEA",border:"1.5px solid "+G.red,borderRadius:10,padding:"8px 12px",fontSize:12.5,color:G.red,fontWeight:600}}>{"⚠️ Dias sem dentista: "+gaps.join(", ")}</div>}<div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>{dOrder.map(function(dia){var on=selDia===dia;var hoje=new Date().getDay()===dia;var n=dayEntries(dia).length;return <button key={dia} onClick={function(){setSelDia(dia);}} style={{flexShrink:0,border:"2px solid "+(on?G.primary:G.border),background:on?G.primary:"#fff",color:on?"#fff":G.text,borderRadius:10,padding:"8px 13px",fontSize:13,fontWeight:700,cursor:"pointer",position:"relative"}}>{DOW_LABELS[dia]}{n>0&&<span style={{marginLeft:5,fontSize:11,opacity:.85}}>{n}</span>}{hoje&&<span style={{position:"absolute",top:-3,right:-3,width:8,height:8,borderRadius:"50%",background:on?"#fff":G.success}}/>}</button>;})}</div><div style={{background:G.card,border:"1px solid "+G.border,borderRadius:14,padding:"14px 16px",display:"flex",flexDirection:"column",gap:14}}><div style={{display:"flex",alignItems:"center"}}><div style={{fontWeight:700,fontSize:15,color:G.primary}}>{DOW_FULL[selDia]}</div>{isAdmin&&selEnts.length>0&&<button onClick={function(){setCopySel({});setCopyOpen(true);}} style={{marginLeft:"auto",border:"1.5px solid "+G.primary,background:"transparent",color:G.primary,borderRadius:8,padding:"5px 11px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📋 Copiar p/ outros dias</button>}</div><div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,marginBottom:6,letterSpacing:".4px"}}>DENTISTAS</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{selDent.length?selDent.map(entryCard):<div style={{fontSize:12.5,color:G.red,fontWeight:600}}>Sem dentista neste dia</div>}</div>{isAdmin&&<button onClick={function(){openAdd("dent");}} style={{marginTop:8,border:"1.5px dashed "+G.primary,background:"transparent",color:G.primary,borderRadius:9,padding:"9px",fontSize:12.5,fontWeight:700,cursor:"pointer",width:"100%"}}>+ Adicionar dentista</button>}</div><div><div style={{fontSize:10.5,fontWeight:700,color:G.muted,marginBottom:6,letterSpacing:".4px"}}>EQUIPE / RECEPÇÃO</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{selEq.length?selEq.map(entryCard):<div style={{fontSize:12.5,color:G.muted}}>—</div>}</div>{isAdmin&&<button onClick={function(){openAdd("equipe");}} style={{marginTop:8,border:"1.5px dashed "+G.primary,background:"transparent",color:G.primary,borderRadius:9,padding:"9px",fontSize:12.5,fontWeight:700,cursor:"pointer",width:"100%"}}>+ Adicionar à equipe</button>}</div></div>{addOpen&&<Modal open={true} close={function(){setAddOpen(null);}} title={(addOpen==="dent"?"Adicionar dentista":"Adicionar à equipe")+" · "+DOW_FULL[selDia]} ch={<div style={{display:"flex",flexDirection:"column",gap:12}}><div><label style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:".4px",display:"block",marginBottom:6}}>{addOpen==="dent"?"Dentista":"Pessoa"}</label><select value={af.pid} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{pid:v,nome:""});});}} style={{width:"100%",border:"1.5px solid "+G.border,borderRadius:9,padding:"11px 12px",fontSize:14,outline:"none",background:"#fff",boxSizing:"border-box"}}><option value="">— selecionar —</option>{(addOpen==="dent"?(dents||[]):(users||[])).map(function(x){return <option key={x.id} value={x.id}>{x.name}</option>;})}<option value="__outro__">Outro (digitar nome)</option></select></div>{af.pid==="__outro__"&&<div><label style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:".4px",display:"block",marginBottom:6}}>Nome</label><input value={af.nome} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{nome:v});});}} style={{width:"100%",border:"1.5px solid "+G.border,borderRadius:9,padding:"11px 12px",fontSize:14,outline:"none",boxSizing:"border-box"}}/></div>}<div style={{display:"flex",gap:10}}><div style={{flex:1}}><label style={{fontSize:11,fontWeight:700,color:G.muted,letterSpacing:".4px"}}>ENTRA</label><input type="time" value={af.inicio} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{inicio:v});});}} style={{width:"100%",border:"1.5px solid "+G.border,borderRadius:9,padding:"10px 12px",fontSize:14,outline:"none",boxSizing:"border-box"}}/></div><div style={{flex:1}}><label style={{fontSize:11,fontWeight:700,color:G.muted,letterSpacing:".4px"}}>SAI</label><input type="time" value={af.fim} onChange={function(e){var v=e.target.value;setAf(function(p){return Object.assign({},p,{fim:v});});}} style={{width:"100%",border:"1.5px solid "+G.border,borderRadius:9,padding:"10px 12px",fontSize:14,outline:"none",boxSizing:"border-box"}}/></div></div><div style={{fontSize:11.5,color:G.muted,lineHeight:1.4}}>Plantão que vira o dia: a saída pode ser menor que a entrada (ex.: 19:00 → 07:00).</div><Btn ch="Adicionar" onClick={confirmAdd}/></div>}/>}{copyOpen&&<Modal open={true} close={function(){setCopyOpen(false);}} title={"Copiar "+DOW_FULL[selDia]+" para…"} ch={<div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{fontSize:12.5,color:G.muted,lineHeight:1.45}}>{"Marque os dias que vão receber a mesma escala de "+DOW_FULL[selDia]+". Substitui o que houver nesses dias."}</div>{dOrder.filter(function(d){return d!==selDia;}).map(function(dia){var on=!!copySel[dia];return <button key={dia} onClick={function(){setCopySel(function(p){var nn=Object.assign({},p);nn[dia]=!nn[dia];return nn;});}} style={{display:"flex",alignItems:"center",gap:9,border:"1.5px solid "+(on?G.primary:G.border),background:on?G.accent:"#fff",borderRadius:9,padding:"10px 12px",fontSize:13.5,fontWeight:600,cursor:"pointer",color:G.text,textAlign:"left"}}><span style={{width:18,height:18,borderRadius:5,flexShrink:0,background:on?G.primary:"#fff",border:"2px solid "+(on?G.primary:G.border),color:"#fff",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center"}}>{on?"✓":""}</span>{DOW_FULL[dia]}</button>;})}<Btn ch="Aplicar cópia" onClick={applyCopy}/></div>}/>}</div>;}
...
var ST_COL={canal:"#D6453F",coroa:"#C9A227",nucleo:"#6E7F8D",implante:"#12968C",fratura:"#E0902A",ausente:"#9DACA4",extrair:"#D6453F"};
var SCREW="#8896A0",IVORY="#FBF8F0",TLINE="#C3D1C9",DIV="#E3EBE6",GOLD="#B7950B",NUC="#6E7F8D";
...
function toothEls(key,cx,occ,up,w,hc,hr,nroots,canine,cond){
 ...
 if(has("nucleo")&&!faded&&!has("implante")){var pw=Math.max(3,w*0.17),py0=g.cerv,py1=g.cerv+g.sgn*(hr*0.5);e.push(<rect key={key+(k++)} x={cx-pw/2} y={Math.min(py0,py1)} width={pw} height={Math.abs(py1-py0)} rx={pw/2} fill={NUC} stroke="#566773" strokeWidth={0.6}/>);}
 ...
 if(faded){var lo=Math.min(g.cerv,g.apex,occ),hi=Math.max(g.cerv,g.apex,occ);
  e.push(<line key={key+(k++)} x1={cx-g.half} y1={lo} x2={cx+g.half} y2={hi} stroke="#9DACA4" strokeWidth={2.2} strokeLinecap="round"/>);
  e.push(<line key={key+(k++)} x1={cx+g.half} y1={lo} x2={cx-g.half} y2={hi} stroke="#9DACA4" strokeWidth={2.2} strokeLinecap="round"/>);}
 return e;
}
...
function archEls(){
 ...
 ARCH.up.forEach(function(n,i){var cx=colx(i),m=meta(n),rec=getRec(n),cd=getCond(rec);
  if(sel===n)e.push(<rect key={"hu"+n} x={cx-col/2+2} y={6} width={col-4} height={svgH/2-2} rx={9} fill="#E3EFE9"/>);
  ...
  var dn=decidOf(n);if(dn&&mode==="perm")e.push(<text key={"du"+n} x={cx} y={uN2} fontFamily="'Cormorant Garamond'" fontSize={10} fill="#9CB0A8" textAnchor="middle">{dn}</text>);
 ...
 ARCH.lo.forEach(function(n,i){var cx=colx(i),m=meta(n),rec=getRec(n),cd=getCond(rec);
  if(sel===n)e.push(<rect key={"hl"+n} x={cx-col/2+2} y={svgH/2} width={col-4} height={svgH/2-8} rx={9} fill="#E3EFE9"/>);
  ...
  var dn=decidOf(n);if(dn&&mode==="perm")e.push(<text key={"dl"+n} x={cx} y={lN2} fontFamily="'Cormorant Garamond'" fontSize={10} fill="#9CB0A8" textAnchor="middle">{dn}</text>);
 });
 var mx=colx(halfn-1)+col/2+midgap/2;
 e.push(<line key="mid" x1={mx} y1={10} x2={mx} y2={svgH-10} stroke="#D7E2DC" strokeWidth={1} strokeDasharray="2 4"/>);
...
function inspector(){
 ...
 var content=(<div style={{display:"flex",flexDirection:"column",gap:14}}>
   ...
   <div>
     ...
     <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{ST_OPTS.map(function(o){var on=o[0]===""?cd.length===0:cd.indexOf(o[0])>=0;var c=ST_COL[o[0]]||G.primary;return <button key={o[0]||"norm"} onClick={function(){toggleCond(sel,o[0]);}} style={{border:"2px solid "+(on?c:G.border),background:on?c:"#fff",color:on?"#fff":G.text,borderRadius:9,padding:"7px 12px",fontSize:12.5,fontWeight:600,cursor:"pointer"}}>{o[1]}</button>;})}</div>
   </div>
   <div>
...
var legend=[["Cárie","#D6453F","fill"],["Restauração","#2E78B8","fill"],["Canal","#D6453F","line"],["Coroa","#C9A227","ring"],["Núcleo","#6E7F8D","fill"],["Implante","#12968C","fill"],["Fratura","#E0902A","fill"],["Ausente","#9DACA4","x"],["Extração","#D6453F","x"]];

return (<div style={{display:"flex",flexDirection:"column",gap:12}}>
 <div style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:10}}>
   <div style={{fontWeight:700,fontSize:15,color:G.primary}}>🦷 Odontograma</div>
   <div style={{marginLeft:"auto",display:"flex",background:"#E7EFEA",borderRadius:16,padding:2}}>{[["perm","Permanente"],["dec","Decíduo"]].map(function(o){var on=mode===o[0];return <button key={o[0]} onClick={function(){setMode(o[0]);setSel(null);}} style={{border:"none",background:on?G.primary:"transparent",color:on?"#fff":G.muted,borderRadius:14,padding:"6px 14px",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>{o[1]}</button>;})}</div>
 </div>
...
// ── PORTAL: recalcula o resumo _portal dos pacientes com token ──
useEffect(function(){
  if(!initialized.current)return;
  var td=today();var changed=false;
  var next=pats.map(function(p){
    if(!p||!p.portalToken)return p;
    var core=buildPortalSnapshot(p,p.portalOpts||PORTAL_DEFAULT_OPTS,{appts:appts,treats:treats,budgets:budgets,dents:dents,clinica:CLINICA_LIVE,today:td});
    var prevCore=p._portal?Object.assign({},p._portal):null;if(prevCore)delete prevCore.updatedAt;
    if(JSON.stringify(prevCore)!==JSON.stringify(core)){changed=true;return Object.assign({},p,{_portal:Object.assign({},core,{updatedAt:new Date().toISOString()})});}
    return p;
  });
  if(changed)setPats(next);
},[pats,appts,treats,budgets,dents]);

// ── PORTAL: reconcilia confirmações de presença (janela de 3 dias) ──
useEffect(function(){
  if(!SUPA_URL)return;
  var run=function(){
    if(!initialized.current||document.hidden)return;
    supabase.fetchPortalActions().then(function(rows){
      if(!rows||!rows.length)return;
      var td=today();var alvo={};
      rows.forEach(function(r){var a=r.action||{};if(a.type!=="confirm")return;if(r.created_at){var age=(Date.now()-new Date(r.created_at).getTime())/86400000;if(age>3)return;}if(a.apptId!=null)alvo[a.apptId]=true;});
      if(!Object.keys(alvo).length)return;
      setAppts(function(prev){var did=false;var nx=prev.map(function(ap){if(alvo[ap.id]&&ap.status!=="confirmed"&&ap.status!=="done"&&ap.date>=td){did=true;return Object.assign({},ap,{status:"confirmed",statusTs:new Date().toISOString(),confirmedByPortal:true});}return ap;});return did?nx:prev;});
    }).catch(function(){});
  };
  var t0=setTimeout(run,6000);var iv=setInterval(run,30000);
  return function(){clearTimeout(t0);clearInterval(iv);};
},[]);
...
<div style={{display:"flex",minHeight:"100vh"}}>
  ...
  <div className={`sidebar${sideOpen?"":" closed"}`} style={{background:"linear-gradient(180deg,rgba(23,47,38,.90),rgba(11,30,23,.93))",backdropFilter:"blur(22px) saturate(140%)",WebkitBackdropFilter:"blur(22px) saturate(140%)",borderRight:"1px solid rgba(255,255,255,.08)",borderRadius:"0 18px 18px 0",boxShadow:"3px 0 22px rgba(0,0,0,.28)",display:"flex",flexDirection:"column",padding:"14px 10px",gap:2,flexShrink:0}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"6px 4px 14px",flexShrink:0}}>
      <div>
        <div style={{fontFamily:"'Cormorant Garamond'",fontSize:18,color:"#fff",lineHeight:1.2}}>{"🦷 "+CLINICA_LIVE.nome}</div>
    ...
    <div style={{flexShrink:0,borderTop:"1px solid rgba(255,255,255,.12)",paddingTop:10,marginTop:6}}>
      ...
      <button onClick={()=>{setShowWelcome(true);setSideOpen(false);}} style={{border:"none",background:"rgba(255,255,255,.1)",borderRadius:8,padding:"6px 11px",color:"rgba(255,255,255,.8)",fontSize:11,fontWeight:600,cursor:"pointer",width:"100%",textAlign:"left",marginBottom:5}}>❓ Ajuda / Tour</button>
      <button onClick={()=>{var n=(CLINICA_LIVE.whatsapp||"").replace(/\D/g,"");window.open("https://wa.me/"+(SUPORTE_WA)+"?text="+encodeURIComponent("Olá! Preciso de ajuda com o sistema Orbe."),"_blank");}} style={{border:"none",background:"rgba(255,255,255,.1)",borderRadius:8,padding:"6px 11px",color:"rgba(255,255,255,.8)",fontSize:11,fontWeight:600,cursor:"pointer",width:"100%",textAlign:"left",marginBottom:5}}>💬 Falar com suporte</button>
      <button onClick={()=>{try{localStorage.removeItem("orbe_token");localStorage.removeItem("orbe_user");}catch(e){}location.reload();}} style={{border:"none",background:"rgba(255,255,255,.1)",borderRadius:8,padding:"6px 11px",color:"rgba(255,255,255,.7)",fontSize:11,fontWeight:600,cursor:"pointer",width:"100%",textAlign:"left"}}>🚪 Sair</button>
    </div>
  </div>
...
{/* Main content */}
  ...
  <div className="main-content" style={{flex:1,overflowY:"auto",minWidth:0}}>
    ...
    <div style={{padding:"16px",paddingTop:view==="agenda"?"84px":"16px"}}>
      {dicas&&HINTS[view]&&<div style={{background:"#EAF4EE",border:"1px solid "+G.border,borderRadius:12,padding:"11px 14px",marginBottom:14,display:"flex",gap:9,alignItems:"flex-start",color:G.primary,fontSize:13,lineHeight:1.45}}><span style={{fontSize:15,flexShrink:0}}>💡</span><span>{HINTS[view]}</span></div>}
