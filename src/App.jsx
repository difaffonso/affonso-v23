...
{isAdmin&&fixos.length>0&&gapDias>0&&<div style={{background:"#FDECEA",border:"1.5px solid "+G.red,borderRadius:10,padding:"8px 12px",fontSize:12.5,color:G.red,fontWeight:600}}>{"⚠️ "+gapDias+(gapDias===1?" dia sem dentista neste mês":" dias sem dentista neste mês")}</div>}
...
const [escala,setEscala]=useState(function(){try{var v=JSON.parse(localStorage.getItem("orbe_escala_v4")||"null");if(v&&v.fixos)return {fixos:v.fixos||[],deltas:v.deltas||{}};}catch(e){}return {fixos:[],deltas:{}};});
useEffect(function(){try{localStorage.setItem("orbe_escala_v4",JSON.stringify(escala));}catch(e){}},[escala]);
