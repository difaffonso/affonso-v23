...
{TABS.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{border:"none",background:tab===k?G.primary:"#f0f4f2",color:tab===k?"#fff":G.muted,borderRadius:"8px 8px 0 0",padding:"9px 16px",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s",marginBottom:-2,borderBottom:tab===k?"2px solid "+G.primary":"none"}}>{l}</button>)}
    ...
    <div style={{padding:22}}>
      ...
      {tab==="ficha"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {pat.obs&&<div style={{background:G.yellow+"18",border:"2px solid "+G.yellow",borderRadius:10,padding:"9px 14px"}}><span style={{fontWeight:700,color:G.yellow}}>ATENCAO ALERGIA / OBS. IMPORTANTE</span><div style={{color:G.text,marginTop:4,fontSize:14}}>{pat.obs||pat.allergy}</div></div>}
        ...
        {editMode&&<Txt lb="ATENCAO Obs. Importante (destaque em toda a clínica)" val={pf.obs} set={v=>setPf(p=>({...p,obs:v}))} rows={2}/>}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[["hypertension","Hipertensão"],["diabetes","Diabetes"],["heartDisease","Cardiopatia"],["bleeding","Coagulação"],["osteoporosis","Osteoporose"],["kidneyDisease","Doença Renal"],["liverDisease","Doença Hepática"],["thyroid","Tireóide"],["epilepsy","Epilepsia"],["cancer","Câncer/Quimio"],["pregnant","Gestante"],["smoking","Tabagismo"]].map(([k,l])=>{
            ...
            return <label key={k} style={{display:"flex",alignItems:"center",gap:9,background:v?G.red+"12":G.bg,borderRadius:9,padding:"10px 13px",cursor:"pointer",border:"1.5px solid "+v?G.red:G.border"}}>
              ...
              {v&&<span style={{marginLeft:"auto",fontSize:11,color:G.red,fontWeight:700}}>ATENCAO Sim</span>}
            </label>;
          })}
        </div>
...
<div style={{display:"flex",flexDirection:"column",gap:10}} className="fi">
  ...
  {vd.length===1&&<div style={{display:"flex",flexDirection:"column",gap:3}}>
    {SLOTS.map(function(slot){
      ...
      var flags=[];
      if(p&&p.obs)flags.push("ATENCAO️ "+p.obs);
        ...
        return (
          <div key={slot} style={{display:"grid",gridTemplateColumns:"48px repeat("+vd.length+",1fr)",gap:2,marginBottom:2,minHeight:hasAny?0:36}}>
            ...
            {vd.map(d=>{
              ...
              const healthFlags=[p&&p.obs&&("ATENCAO "+p.obs),p&&p.allergy&&p.allergy!=="Nenhuma"&&("💊 "+p.allergy),an.allergicMeds&&("💊 Alergia Med: "+an.allergicMeds),...CONDS.filter(([k])=>an[k]).map(([,l])=>l)].filter(Boolean);
              if(a&&p)return(
                <div key={d.id} onClick={()=>setViewA(a)} style={{background:SC[a.status]+"18",border:"2px solid "+SC[a.status],borderRadius:8,padding:"5px 8px",cursor:"pointer",minHeight:48}}>
                  ...
                  {healthFlags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:2}}>{healthFlags.map(function(f,i){return <span key={i} style={{fontSize:8,background:f.startsWith("ATENCAO")?G.red+"20":f.startsWith("💊")?G.yellow+"20":G.blue+"15",color:f.startsWith("ATENCAO")?G.red:f.startsWith("💊")?G.yellow:G.blue,borderRadius:3,padding:"1px 4px",fontWeight:700}}>{f}</span>;})}</div>}
    ...
    return(
      <Modal open close={function(){setViewA(null);setHistTab("info");}} title="Consulta" wide ch={
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          {p&&p.obs&&<div style={{background:G.yellow+"18",border:"2px solid "+G.yellow,borderRadius:10,padding:"8px 12px",fontWeight:700,color:G.yellow}}>{"ATENCAO "+p.obs}</div>}
...
{p.obs&&<div style={{background:G.red+"20",border:"1px solid "+G.red",borderRadius:5,padding:"2px 7px",fontSize:10,fontWeight:700,color:G.red,marginTop:2,display:"inline-block"}}>ATENCAO {p.obs.slice(0,45)}</div>}
{(p.allergy&&p.allergy!=="Nenhuma"&&!p.obs)&&<div style={{background:G.yellow+"20",border:"1px solid "+G.yellow",borderRadius:5,padding:"2px 7px",fontSize:10,fontWeight:700,color:G.yellow,marginTop:2,display:"inline-block"}}>ATENCAO {p.allergy}</div>}
</div>
...
<Modal open={pm} close={()=>setPm(false)} title={ep?"Editar Paciente":"Novo Paciente"} wide ch={<div style={{display:"flex",flexDirection:"column",gap:11}}>
  ...
  <Txt lb="ATENCAO Obs. Importante (alergia grave, destaque vermelho)" val={pf.obs} set={fp("obs")} rows={2}/>
...
<div style={{display:"flex",gap:6,alignItems:"center",marginBottom:3,flexWrap:"wrap"}}><span style={{fontWeight:700,fontSize:13}}>{pat?.name}</span><span style={{fontSize:11,color:G.muted}}>P.{pat?.folder}</span><Bdg l={PROS_SL[p.status]} col={PROS_SC[p.status]} sm/>{late&&<Bdg l="ATENCAO ATRASADO" col={G.red} sm/>}{isT&&!late&&<Bdg l="📅 HOJE" col={G.orange} sm/>}</div>
...
{vencHoje?"ATENCAO️ Vence HOJE!":vencAmanha?"ATENCAO️ Vence amanhã!":"Válido até "+fmt(e.valido)}
</div>
</div>
...
{id:"retorno",  label:"ATENCAO️ Retorno Tratamento",msg:mk(["Olá, {nome}! 😊","","Notamos que você está em tratamento conosco e ainda não remarcou sua próxima consulta. Que tal agendarmos? 😊","","Estamos aqui para você!","","Affonso Odontologia"])},
];
...
{id:"sem_ret",label:"ATENCAO️ Em tratamento sem agendamento",col:G.red,list:semRetorno,sub:"Plano ativo sem consulta futura",extra:x=>{const pend=x.items.filter(i=>!i.done).length;return"Plano: "+x.name+" · "+pend+" proc. pendente"+pend>1?"s":";},wa:waSemRet,isTreat:true},
...
{low&&<div style={{background:G.red+"15",borderRadius:6,padding:"3px 8px",fontSize:10,fontWeight:700,color:G.red,marginTop:5}}>ATENCAO Estoque baixo!</div>}
...
{hasAlert&&<div style={{fontSize:10,background:G.red+"20",color:G.red,borderRadius:4,padding:"1px 6px",fontWeight:700,marginTop:1,display:"inline-block"}}>ATENCAO Atenção</div>}
...
{!isDent&&<Sel lb="" val={selDent} set={setSelDent} opts={myDents.map(function(d){return{v:String(d.id),l:d.name};})}/>}
  ...
  {missing.length>0&&(
    <div style={{background:G.red+"12",border:"2px solid "+G.red,borderRadius:12,padding:"12px 14px"}}>
      <div style={{fontWeight:700,color:G.red,fontSize:13,marginBottom:6}}>{"ATENCAO Falta de crédito — "+missing.length+" procedimento(s) sem baixa:"}</div>
